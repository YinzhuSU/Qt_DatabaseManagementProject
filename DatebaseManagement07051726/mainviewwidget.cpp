    ...
    ui(new Ui::MainViewWidget)
{
    ...
    ui->setupUi(this);
    this->setWindowIcon(QIcon(":/new/prefix1/Images/mainview.png"));
...
void MainViewWidget::on_btnSaveOne_clicked()
{
    ...
    if(!file.open(QFile::ReadOnly | QFile::Text)) //these enums are important
    {
        ...
        //save to database:
        QString total = ui->pteTotalView->toPlainText();
        QString title = total.section("\n\n", 0, 0);
        QString content = total.section("\n\n", 1);
    ...
    else
    {
        ...
        if(qry->exec())
        {
            if(qry->next()){
                ...
                //                QString total = ui->pteTotalView->toPlainText();
                //                titleBefore = total.section("\n\n", 0, 0);
                //                contentBefore = total.section("\n\n", 1);
...
void MainViewWidget::on_btnUpdate_clicked()
{
    ...
    else
    {
        ...
        //修改前把数据先备份到成员变量的三个QString里：
        qry->prepare("SELECT * FROM EntertainmentData WHERE Filename = :filename");
        qry->bindValue(":filename", filename+".txt");
        if(qry->exec())
        {
            if(qry->next()){
                totalBefore = qry->value(1).toString() +"\n\n"+ qry->value(2).toString();
                titleBefore = totalBefore.section("\n\n", 0, 0);
                contentBefore = totalBefore.section("\n\n", 1);
            }else{
                QString warningContent = "There is no file whose name is "+filename+" in your database."
                                                                                    "\n"
                                                                                    "Please import it first.";
        ...
        //修改后：
        QString total = ui->pteTotalView->toPlainText();
        QString titleAfter = total.section("\n\n", 0, 0);
        QString contentAfter = total.section("\n\n", 1);
        filename = ui->leViewFile->text();
        ...
        qry->bindValue(":title", titleAfter);
        qry->bindValue(":content", contentAfter);
        ...
        //由于使用SQLite的trigger总是失败，被迫无奈手动创建system log：
        qry->prepare("INSERT INTO SystemLog(AdminUsername, OperatingTime, OperatedFilename, OperationType, "
                     "TitleBeforeOperation, TitleAfterOperation, ContentBeforeOperation, ContentAfterOperation) "
                     "VALUES(:admin, :opTime, :opFilename, :opType, :titleBefore, :titleAfter, :contentBefore, :contentAfter)");
        qry->bindValue(":admin", username); //来自成员变量
        qry->bindValue(":opTime", lastModificationTime); //updateTime
        qry->bindValue(":opFilename", filename+".txt");
        qry->bindValue(":opType", "UPDATE");
        qry->bindValue(":titleBefore", titleBefore);
        qry->bindValue(":titleAfter", titleAfter);
        qry->bindValue(":contentBefore", contentBefore);
        qry->bindValue(":contentAfter", contentAfter);
        if(qry->exec())
        {
            qDebug() << "Update log has been recorded.";
            connClose();//We should close the connection of database when we finish the operations
        }
        else
        {
...
void MainViewWidget::on_btnDelete_clicked()
{
    if(ui->leViewFile->text() == NULL)
    {
        ...
        QDateTime dateTime = QDateTime::currentDateTime();
        QString deleteTime = dateTime.toString();
        QString filename = ui->leViewFile->text();
        QString total = ui->pteTotalView->toPlainText();
        QString title = total.section("\n\n", 0, 0);
        QString content = total.section("\n\n", 1);
        ...
        //由于使用SQLite的trigger总是失败，被迫无奈手动创建system log：
        qry->prepare("INSERT INTO SystemLog(AdminUsername, OperatingTime, OperatedFilename, OperationType, TitleBeforeOperation, ContentBeforeOperation) "
                     "VALUES(:admin, :opTime, :opFilename, :opType, :titleBefore, :contentBefore)");
        {
            qry->bindValue(":admin", username); //来自成员变量
            qry->bindValue(":opTime", deleteTime);
            qry->bindValue(":opFilename", filename+".txt");
            qry->bindValue(":opType", "DELETE");
            qry->bindValue(":titleBefore", title );
            qry->bindValue(":contentBefore", content);
        }
        if(qry->exec())
        {
            qDebug() << "Delete log has been recorded.";
        }
        else
        {
            QMessageBox::critical(this, tr("Error Message"),qry->lastError().text());
...
void MainViewWidget::on_btnTest_clicked()
{
    ui->lblTest->setText(this->windowTitle());
}
