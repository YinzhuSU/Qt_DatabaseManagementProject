    ...
    ui(new Ui::LoginWidget)
{
    ...
    this->setWindowIcon(QIcon(":/new/prefix1/Images/login.png"));
    ...
    if(username == NULL || password == NULL)
    {
        ...
        if(qry->exec("SELECT * FROM UserInfo WHERE Username = '"+ username +"' and Password = '"+ password +"'"))
        {
            ...
            if(count >= 1) //correct username and password, in fact, it cannot more than 1
            {
                ...
                ui->lePassword->setText(NULL);
                mainView->show();
                mainView->username = username;//把username的信息传到下一层（mainView）
                ///7//One method to control mainView's buttons on LoginView:
                if(qry->value(4).toString() != "Admin"){
                    mainView->hideForMember();
                }else{
                    mainView->showForAdmin();
                }
            }
            else
            {
